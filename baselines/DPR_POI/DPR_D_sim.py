...
if 'GeoGLUE' in dataset:
    # Grid search shows that alpha = 0.9 is the best for GeoGLUE queries
    alpha = 0.9
else:
...
if portion == '1':
    np.save(f'result/{dataset}_DPR_D_top100.npy', top_k_indices)
else:
    np.save(f'result/{dataset}_DPR_D_{portion}_top100.npy', top_k_indices)