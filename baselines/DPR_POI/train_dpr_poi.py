...
class POIDataset(torch.utils.data.Dataset):
    def __init__(self, query_path, poi_path, hard_negative_path):
        self.queries = []
        self.poi_data = []
        self.hard_negatives = np.load(hard_negative_path)

        # Load and process query data
        with open(query_path, 'r', encoding='utf-8') as f:
            for line in f:
                parts = line.strip().split('\t')
                # Assuming the format is: query,lat,lon,poi_id(s)
                self.queries.append({'query': parts[0], 'poi_ids': [int(x) for x in parts[3].split(',')]})

        # Load and process POI data
        with open(poi_path, 'r', encoding='utf-8') as f:
            for line in f:
                parts = line.strip().split('\t')
                # Assuming the format is: poi_name,lat,lon
                self.poi_data.append(parts[0])

        # Prepare the dataset
        self.data = []
        for idx, query in enumerate(self.queries):
            positive_set = set(query['poi_ids'])
            positive_pois = [self.poi_data[pid] for pid in positive_set]
            positive_name_set = set(positive_pois)
            hard_negatives_idxs = self.hard_negatives[idx]

            hard_negative_pois = []
            for neg_id in hard_negatives_idxs:
                if neg_id not in  positive_set:
                    negative_poi = self.poi_data[neg_id]
                    if negative_poi not in positive_name_set:
                        hard_negative_pois.append(negative_poi)
                        if len(hard_negative_pois) >= 10:
                            break

            self.data.append({
                'query': query['query'],
                'positive_pois': positive_pois,
                'hard_negative_pois': hard_negative_pois
            })
    ...
    def collate_fn(samples, tokenizer, args, stage):
        ...
        # Prepare POI inputs (both positive and hard negatives)
        positive_passages = [random.choice(sample['positive_pois']) for sample in samples]
        if stage == 'train':
            negative_passages = [random.choice(sample['hard_negative_pois']) for sample in samples]
        else:
            negative_passages = [sample['hard_negative_pois'][:min(args.num_hard_negative_ctx, len(sample['hard_negative_pois']))] for sample in samples]
            negative_passages = [x for y in negative_passages for x in y]

        all_pois = positive_passages + negative_passages
        # Tokenize all POIs together
        doc_inputs = tokenizer(all_pois, max_length=256, padding=True, truncation=True, return_tensors='pt')

        # Note: Depending on your model's architecture, you might need to adjust how you organize
        # the returned dictionary, especially if your model expects separate inputs for positive
        # and negative contexts or has other specific requirements.
        return {
...
def main():
    ...
    train_dataset = POIDataset(args.train_file, args.poi_file, args.train_neg_file)
    train_collate_fn = functools.partial(POIDataset.collate_fn,tokenizer=tokenizer,stage='train',args=args,)
    ...
    dev_dataset = POIDataset(args.dev_file, args.poi_file, args.dev_neg_file)
    dev_collate_fn = functools.partial(POIDataset.collate_fn,tokenizer=tokenizer,stage='dev',args=args,)
