...
poi_embeddings = np.load(f'baselines/BERT/embeddings/{dataset}/poi_embeddings.npy')
test_embeddings = np.load(f'baselines/BERT/embeddings/{dataset}/test_embeddings.npy')

# for each test embedding, find the top k poi embeddings with the highest cosine similarity

import torch

poi_embeddings = torch.tensor(poi_embeddings).cuda()
...
for i in trange(test_embeddings.size(0)):
    ...
    # Manual check: What is the top 10, in text?
    # print(f'Query: {query_txt[i]}')
    # print([poi_txt[i] for i in top_indices[:10]])

# save it to the result file
    
top_k_indices = np.array(top_k_indices)

np.save(f'result/{dataset}_BERT_D_top100.npy', top_k_indices)