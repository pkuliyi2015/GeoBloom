...
public:
    ...
    void set_leaf_node(uint32_t* bits, vector<uint16_t>& bloom_filter, pair<double, double>& location, uint32_t node_idx) {
        this->bloom_filter.bits = bits;
        for (uint16_t i : bloom_filter) {
            this->bloom_filter.set(i);
        }
...
public:
    ...
    inline void eval_intersect(
        ...
        l3_avx2(node->bloom_filter.bits, query_length, query_numbers, depth, rank_hidden, context_select_hidden, intersection_score, context_select_score, unmatched_bits_length, unmatched_bits);
...
public:
    ...
    Tree(Dataset &poi, vector<vector<vector<uint32_t>>> &levels, int keep_depth){
        ...
        int total_rows = poi.num_rows;
        this->sizes.push_back(total_rows);
        for (int depth = 1; depth < keep_depth; ++depth) {
            total_rows += levels[depth-1].size();
            sizes.push_back(levels[depth-1].size());
        }
        cout << "Allocating " << float(sizeof(uint32_t) * BF_SIZE * total_rows ) / 1024 / 1024 << " MB for "<< total_rows << " bloom filters..." << endl;
        this->bf_bits = (uint32_t *) aligned_alloc(64, sizeof(uint32_t) * total_rows * BF_SIZE);
        memset(this->bf_bits, 0, sizeof(uint32_t) * total_rows * BF_SIZE);
        ...
        for (uint32_t i = 0; i < poi.num_rows; ++i) {
            leaf_nodes[i].set_leaf_node(ptr, poi.bloom_filters[i], poi.locations[i], i);
            ptr += BF_SIZE;
        }
        ...
        for (int depth = 0; depth < keep_depth; ++depth) {
    ...
    vector<uint32_t> beam_search_nnue(NNUE & nnue, vector<uint16_t> query_bloom_filter, double lat, double lon, vector<uint32_t> & beam_width, sort_buffer & buffer, int topk){
        ...
        for (int i = 0; i < write_ptr; ++i) {
            ...
            for (int j = 0; j < top_context; ++j) {
                ...
                int context_score;
                nnue.eval_context(
                    context_node->bloom_filter.bits, 
                    unmatched_length,
